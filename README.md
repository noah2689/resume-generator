...
> M2 尚未通过验收。M2 的完整验收标准是「新增一条工作经历后预览立即出现、删除后预览消失」，需要教育 / 工作 / 项目 / 技能四类 CRUD 与 Section 显示隐藏，目前只完成了基本信息这一小步。
...
编辑器页当前能做的事只有两件：**读取示例数据渲染 A4 预览**，以及**编辑 5 个基本信息字段并实时看到预览变化**。它仍然没有内容增删改、没有样式设置、没有保存。
...
| `/` | 我的简历 | 骨架 |
| `/new` | 新建简历 | 骨架 |
| `/editor/:resumeId` | 简历编辑器 | M2.1：基本信息可编辑 + 实时预览（无持久化、无样式设置） |
