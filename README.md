...
编辑器还**不是**真正的编辑器。以下均为后续阶段内容，当前不存在：
...
编辑器页当前做的事情只有一件：**取到示例数据，交给模板渲染成一份 A4 简历预览**。它只有「预览」这一半，没有编辑与样式设置。
...
结构保持扁平，没有提前划分 `components/`、`hooks/`、`lib/` 等目录。目录分层随真实需求增长，而不是预先猜测。`templates/` 是在 M1 出现第一个真实模板需求时才建立的，且当前只有实际需要的两个文件。
...
| `/` | 我的简历 | 骨架 |
| `/new` | 新建简历 | 骨架 |
| `/editor/:resumeId` | 简历编辑器 | M1 单栏简历预览（只读，无编辑与样式设置） |
